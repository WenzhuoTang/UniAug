...
launch () {
    ...
    # extra_config+="train.batch_size=10000 "
    extra_config+="train.eval_steps=1 "

    ## cflp
    # extra_args+="--prefix gamma-30.0 "
    # extra_config+="cflp.gamma=30.0 "

    # extra_args+="--prefix neg_rate-50 "
    # extra_config+="cflp.neg_rate=50 "

    # extra_args+="--prefix alpha-0.1 "
    # extra_config+="cflp.alpha=0.1 "

    # extra_args+="--prefix beta-0.1 "
    # extra_config+="cflp.beta=0.1 "

    extra_args+="--prefix alpha-1e-3_beta-1e-3 "
    extra_config+="cflp.alpha=1.0e-3 cflp.beta=1.0e-3 "

    # extra_args+="--prefix alpha-10_beta-10 "
    # extra_config+="cflp.alpha=10 cflp.beta=10 "

    # extra_args+="--prefix neg_rate-10_alpha-10 "
    # extra_config+="cflp.neg_rate=10 cflp.alpha=10  "

    # extra_args+="--prefix neg_rate-10_beta-1e-3 "
    # extra_config+="cflp.neg_rate=10 cflp.beta=1.0e-3  "

    # extra_args+="--prefix neg-0.1_alp-1e-3_bet-1e-3 "
    # extra_config+="cflp.neg_rate=0.1 cflp.alpha=1.0e-3 cflp.beta=1.0e-3 "
    ...
    config_file="config/link_pred_cf/${cfg_name}.yaml"

    script="PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True "
    script+="python link_pred_cf.py --config ${config_file} --seed ${seed} ${extra_args} ${extra_config} ${settings[@]} "

    echo ${script} && [[ $TEST_FLAG == 0 ]] && eval ${script}
}


# AUG_FLAG=0 USE_VAL_FLAG=1 CUDA_VISIBLE_DEVICES=0 bash run/link_pred_cf.sh
# AUG_FLAG=1 SEG_FLAG=1 USE_VAL_FLAG=1 CUDA_VISIBLE_DEVICES=0 bash run/link_pred_cf.sh

# AUG_FLAG=0 CUDA_VISIBLE_DEVICES=0 bash run/link_pred_cf.sh
# AUG_FLAG=0 REV_DUP_FLAG=1 CUDA_VISIBLE_DEVICES=0 bash run/link_pred_cf.sh
# AUG_FLAG=1 SEG_FLAG=0 CUDA_VISIBLE_DEVICES=0 bash run/link_pred_cf.sh
# AUG_FLAG=1 SEG_FLAG=1 CUDA_VISIBLE_DEVICES=0 bash run/link_pred_cf.sh
